...
<body>
  <div id="appData" ng-app="appData" ng-controller="GreetingCtrl">
    <div ng-repeat="parentItem in nodeList.groups" style="overflow: scroll;" id="{{parentItem.id}}" class="parentType">
      <div style="width: 100%;line-height:26px;text-align: center;background-color: #ccc;">{{parentItem.id}}</div>
      <div style="background-color: aqua;margin: 10px;" ng-repeat="childGroup in parentItem.childGroup">
        <div style="position: absolute;width: 60px;height: 30px;" id="{{childGroup[0].id}}">{{childGroup[0].id}}</div>
        <div style="position: absolute;width: 60px;height: 30px;" id="{{childGroup[1].id}}">{{childGroup[1].id}}</div>
      </div>
    </div>
    <script>
          ...
          $scope.nodeList.groups.forEach(function (parentItem, parentIndex) {
            ...
            parentItem.childGroup.forEach(function (childItem, childIndex) {
              ...
              firstInstance[parentItem.id].connect({
                target: childItem[0].id,
                source: childItem[1].id,
